# -*- coding: utf-8 -*-
"""31 Revisi-Sistem Rekomendasi Ekowisata di Indonesia.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1GzxXYDvuhpO2hX0Fs6_cC3EwRjGN9UFN
...
### **Persiapan untuk Content-Based Filtering (CBF)**
...
get_recommendations(selected_category='budaya', selected_city='Bandung', same_city_only=True, top_n=5)

get_recommendations(selected_category='budaya', top_n=5)
...
* **Hasil:**
  Sistem menampilkan 5 destinasi wisata dengan kategori yang mengandung kata "budaya" yang semuanya berada di Kota Bandung.
  Contoh tempat: *Stone Garden Citatah*, *Taman Wisata Alam Cikole*, dll.

* **Kesimpulan:**
  Sistem berhasil memfilter berdasarkan kategori dan membatasi hasil hanya dari kota yang diminta.
  Namun, kategori tempat wisata mengandung lebih dari satu kategori (misal: "Budaya,Cagar Alam"), yang berarti rekomendasi masih termasuk destinasi dengan kategori ganda asalkan mengandung kata "budaya".
...
* **Input:**
  Kategori = budaya,
  Kota tidak diisi (None),
  Hanya dari kota yang sama = Tidak

* **Hasil:**
  Sistem merekomendasikan 5 destinasi wisata dengan kategori yang mengandung kata "budaya" dari berbagai kota seperti Sukabumi, Semarang, Bandung, dan Bogor.
  Contoh tempat: *Taman Wisata Alam Situgunung*, *Goa Rong*, dll.

* **Kesimpulan:**
  Ketika kota tidak diberikan, sistem memberikan rekomendasi destinasi wisata berdasarkan kategori tanpa batasan kota.
  Hal ini memungkinkan pengguna melihat rekomendasi terbaik secara nasional (atau dataset lengkap) untuk kategori yang dipilih.
  Kategori destinasi masih bisa berupa gabungan kategori, asalkan salah satu mengandung kata kunci kategori.
...
* Val_loss turun dari 0.3720 → 0.3636, menunjukkan bahwa model makin baik dalam memprediksi rating destinasi.
...
User ID 64 menunjukkan ketertarikan pada wisata alam terbuka seperti taman wisata, air terjun, pantai, hingga kawasan konservasi. Hal ini terlihat dari 5 tempat favoritnya, yaitu:

* Taman Wisata Alam Kampoeng Air Kragilan
* Curug Cimahi
* Taman Bunga Cihideung
* Pantai Klayar
* Ekowisata Mangrove Wonorejo

Tempat-tempat ini memiliki kesamaan berupa suasana alam yang asri, tempat rekreasi luar ruang, serta daya tarik alam seperti bunga, air terjun, dan hutan mangrove.

**2. Rekomendasi Sistem:**

Model merekomendasikan 10 tempat wisata yang belum pernah dikunjungi user, namun sangat relevan dengan preferensi wisata alam yang dimiliki. Di antaranya:

* Wisata alam terbuka: Taman Wisata Alam Situgunung, Sanghyang Heuleut, Air Terjun Kali Pancur
* Taman dan kawasan konservasi: Taman Agrowisata Cilangkap, Taman Wisata Alam Gunung Geulis, Taman Hutan Raya Ngurah Rai
* Wisata edukatif berbasis alam: Desa Wisata Mangunan
* Destinasi danau/waduk dan gua: Waduk Jatibarang Mijen, Gua Pawon
* Taman nasional dan hutan: Taman Nasional Meru Betiri
...
def evaluate_precision_recall_by_category(selected_category, k=5, selected_city=None, same_city_only=False):
    try:
        rekomendasi_df = recommender.recommend_by_category_city(
            selected_category=selected_category,
            selected_city=selected_city,
            top_n=k,
            same_city_only=same_city_only
        ...
        return {
            ...
            'Total Relevant in Dataset': 0
        }

    relevan_count = rekomendasi_df[rekomendasi_df['category'].str.contains(selected_category, case=False, na=False)].shape[0]

    if selected_city and same_city_only:
        total_relevan = place_content_df[
            (place_content_df['category'].str.contains(selected_category, case=False, na=False)) &
            (place_content_df['city'].str.lower() == selected_city.lower())
        ].shape[0]
    else:
        total_relevan = place_content_df[
            place_content_df['category'].str.contains(selected_category, case=False, na=False)
    ...
    precision_at_k = relevan_count / k if k != 0 else 0
...
print(evaluate_precision_recall_by_category('budaya', k=5, selected_city='Bandung', same_city_only=True))
print(evaluate_precision_recall_by_category('budaya', k=5))

"""**Insight Evaluasi Sistem Rekomendasi Berdasarkan Kategori**

Evaluasi 1: Kategori 'budaya' dengan filter kota Bandung dan same\_city\_only=True

* **Precision\@5 = 1.0**
  Semua 5 destinasi yang direkomendasikan benar-benar relevan dengan kategori "budaya". Sistem sangat akurat dalam memilih destinasi yang sesuai kategori saat dibatasi kota.

* **Recall\@5 = 0.5556**
  Dari total 9 destinasi "budaya" yang ada di dataset untuk kota Bandung, sistem berhasil merekomendasikan 5 destinasi. Ini berarti sistem menangkap sekitar 55.56% dari seluruh destinasi budaya yang tersedia di Bandung.

* **Interpretasi:**
  Sistem sangat tepat (precision tinggi) dan sudah mampu menangkap lebih dari setengah destinasi yang relevan di kota tersebut. Namun, ada beberapa destinasi relevan lain yang belum direkomendasikan dalam top 5.


Evaluasi 2: Kategori 'budaya' tanpa filter kota

* **Precision\@5 = 1.0**
  Semua 5 destinasi yang direkomendasikan benar-benar relevan dengan kategori "budaya". Sistem tetap akurat dalam memilih destinasi walau tanpa batasan kota.

* **Recall\@5 = 0.1515**
  Dari total 33 destinasi "budaya" yang ada di dataset (berbagai kota), sistem merekomendasikan 5 destinasi, yang merupakan sekitar 15.15% dari seluruh destinasi budaya.

* **Interpretasi:**
  Sistem tetap presisi tinggi dalam rekomendasi kategori budaya secara global, tetapi recall-nya lebih rendah karena cakupan destinasi yang jauh lebih banyak. Ini wajar karena rekomendasi dibatasi hanya 5 item dari 33 destinasi yang relevan.
